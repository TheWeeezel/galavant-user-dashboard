...
function StatBar({ label, base, added, max, color, colorLight }: StatBarProps) {
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      <span className="w-8 text-m2e-text-secondary font-bold uppercase">{label}</span>
      <div
      ...
      <span className="w-12 text-right text-m2e-text font-bold">{total}/{max}</span>
    </div>
  );
}
    ...
    <div className="pixel-card overflow-hidden hover:border-m2e-accent-dark transition-colors cursor-pointer" onClick={onClick}>
      ...
      <div className="relative aspect-[16/9] bg-m2e-bg-alt border-b-2 border-m2e-border">
        <img
          ...
          loading="lazy"
        />
        <span className="absolute top-2 left-2 px-2 py-0.5 text-xs font-black bg-m2e-card text-m2e-accent pixel-border shadow-sm tracking-wide uppercase">
          #{nft.tokenId}
        </span>
      </div>
      ...
      <div className="p-3 space-y-2">
        <div className="flex items-center justify-between">
          <span className="font-black text-sm uppercase tracking-wide text-m2e-text">{nft.type}</span>
          <span className={`px-2 py-0.5 text-[10px] font-black uppercase pixel-border shadow-sm tracking-wide border-opacity-50 ${qualityColors[nft.quality] ?? qualityColors.common}`}>
            {nft.quality}
          </span>
        </div>
