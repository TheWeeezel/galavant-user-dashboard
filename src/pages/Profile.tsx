...
export function Profile() {
  ...
  if (isRestoring) {
    return (
      <div className="flex items-center justify-center py-20">
        <p className="text-m2e-text-muted font-bold uppercase tracking-widest">Loading...</p>
      </div>
    );
  }
  ...
  if (!isAuthenticated || !user) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-6">
        ...
        <p className="text-lg font-bold uppercase tracking-wider text-m2e-text-secondary">
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto w-full space-y-6">
      ...
      <div className="pixel-card p-5">
        <div className="flex items-center gap-4 mb-4">
          {user.avatarUrl ? (
            ...
            <div className="w-14 h-14 rounded-lg pixel-border border-m2e-border bg-m2e-bg-alt flex items-center justify-center">
              <span className="text-2xl font-black text-m2e-text-secondary">
                {user.nickname[0].toUpperCase()}
              </span>
            </div>
          )}
          <div className="flex-1 min-w-0">
            <p className="text-lg font-black text-m2e-accent uppercase tracking-wider truncate">
              {user.nickname}
            </p>
            <p className="text-xs font-bold text-m2e-text-muted uppercase tracking-wider">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-m2e-text-secondary uppercase tracking-wider">
              Wallet
            </span>
            <span className="text-xs font-bold text-m2e-text truncate flex-1 font-mono">
          ...
          {user.hasGoogleLinked && user.linkedEmail && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-m2e-text truncate flex-1">
                {user.linkedEmail}
              </span>
      ...
      {/* SAT Points Balance */}
      <div className="pixel-card p-5">
        <h2 className="text-sm font-bold uppercase tracking-widest text-m2e-text-secondary mb-3">
          SAT Points Balance
        </h2>
        <div className="flex items-center justify-between">
          <div>
            <span
              className="text-3xl font-black text-m2e-accent"
              style={{ textShadow: '1px 1px 0px var(--color-m2e-accent-dark)' }}
            >
      ...
      {/* Inventory — Bikes */}
      <div className="pixel-card p-5">
        <h2 className="text-sm font-bold uppercase tracking-widest text-m2e-text-secondary mb-4">
          Your Bikes
        </h2>
              ...
              return (
                <div key={bike.id} className="pixel-card overflow-hidden hover:border-m2e-accent-dark transition-colors cursor-pointer" onClick={() => setSelectedBikeId(bike.id)}>
                  <div className="relative aspect-[16/9] bg-m2e-bg-alt border-b-2 border-m2e-border flex items-center justify-center">
                    ...
                    />
                    {bike.isEquipped && (
                      <span className="absolute top-2 right-2 px-2 py-0.5 text-[10px] font-black bg-m2e-success text-m2e-text-on-accent pixel-border shadow-sm tracking-wide border-green-800">
                        Equipped
                      </span>
                    )}
                  </div>
                  <div className="p-3 space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="font-black text-sm uppercase tracking-wide text-m2e-text">{bike.type}</span>
                      <span className={`px-2 py-0.5 text-[10px] font-black uppercase pixel-border shadow-sm tracking-wide border-opacity-50 ${qualityBadge}`}>
                        {bike.quality}
                      </span>
                    </div>
            ...
            {parts.map((part: UserPart) => (
              <div key={part.id} className={`pixel-card overflow-hidden ${PART_TYPE_COLORS[part.type] ?? ''}`}>
                ...
                <div className="px-2 py-2 text-center border-t border-m2e-border">
                  <p className="text-xs font-black uppercase text-m2e-text">
                    {PART_TYPE_LABELS[part.type] ?? part.type}
                  </p>
                  <p className="text-xs font-bold text-m2e-text-muted uppercase">
                    Lv. {part.level}
                  </p>
